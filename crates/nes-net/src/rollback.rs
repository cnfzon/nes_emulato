//! Rollback netplay 的核心排程邏輯（**Phase 4c 才會實作**；4b 只實作 lockstep，見 `session.rs`）。
//!
//! 這個檔案目前只是 Phase 0 留下的資料結構骨架：握手、傳輸、Input／Ack／Checksum、統計與
//! 事件佇列都已經在 4b 的 `LockstepSession` 實作，4c 的 rollback 會沿用那些部分，只替換
//! 「什麼時候可以推進」的排程。
//!
...
mod tests {
    ...
    fn handle_msg_does_not_panic_on_any_variant() {
        ...
        session.handle_msg(Msg::Ack {
            session_id: 1,
            frame: 0,
        });
        session.handle_msg(Msg::Ping {
            session_id: 1,
            timestamp_us: 0,
        });
    }
